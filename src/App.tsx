...
type Marketplace = "meesho" | "flipkart";
...
const FLIPKART_PAGE_WIDTH = 216;
const FLIPKART_PAGE_HEIGHT = 360;
const FLIPKART_CROP = { left: 190 / 595, bottom: 460 / 842, right: 406 / 595, top: 820 / 842 };
...
function extractFlipkartProduct(text: string) {
  const labelRow = text.match(
    /SKU\s*ID\s*\|\s*Description\s+QTY\s+\d+\s+(FREE\s*SIZE|FREE|XXS|XS|S|M|L|XL|XXL|XXXL|XXXXL|XXXXXL|XXXXXXL|[2-7]XL)\s+(.+?)\s+\|\s+/i,
  );
  if (labelRow) {
    return {
      sku: `${labelRow[1]} ${labelRow[2]}`.replace(/\s+/g, " ").trim().toUpperCase(),
      size: cleanSize(labelRow[1]),
    };
  }
  return { sku: "UNKNOWN", size: "UNKNOWN" };
}
...
export default function Home() {
    ...
    try {
      ...
      for (let start = 1; start <= source.numPages; start += batchSize) {
        ...
        await Promise.all(Array.from({ length: end - start + 1 }, async (_, offset) => {
          ...
          const product = marketplace === "flipkart" ? extractFlipkartProduct(text) : extractProductDetails(text);
  ...
  function chooseMarketplace(next: Marketplace) {
    if (next === marketplace) return;
    setMarketplace(next); setFile(null); setRows([]); setStatus("idle"); setMessage("");
    ...
    try {
      ...
      if (marketplace === "flipkart") {
        for (const row of sortedRows) {
          const sourcePage = source.getPage(row.originalIndex);
          const { width, height } = sourcePage.getSize();
          const embedded = await output.embedPage(sourcePage, {
            left: width * FLIPKART_CROP.left,
            bottom: height * FLIPKART_CROP.bottom,
            right: width * FLIPKART_CROP.right,
            top: height * FLIPKART_CROP.top,
          });
          const page = output.addPage([FLIPKART_PAGE_WIDTH, FLIPKART_PAGE_HEIGHT]);
          page.drawPage(embedded, { x: 0, y: 0, width: FLIPKART_PAGE_WIDTH, height: FLIPKART_PAGE_HEIGHT });
        }
        saveBlob(await output.save(), `Flipkart_Labels_3x5_Size_SKU_Sorted_${dateText}.pdf`, "application/pdf");
      } else {
        ...
        saveBlob(await output.save(), `Meesho_Labels_Size_SKU_Sorted_${dateText}.pdf`, "application/pdf");
      }
      setStatus("ready"); setMessage(marketplace === "flipkart" ? "Sorted 3×5 PDF downloaded." : "Sorted PDF downloaded.");
  ...
  function exportExcel() {
    ...
    XLSX.utils.book_append_sheet(workbook, detailSheet, "Size-SKU Picklist"); XLSX.utils.book_append_sheet(workbook, totalSheet, "Size Totals"); XLSX.writeFile(workbook, `${marketplace === "flipkart" ? "Flipkart" : "Meesho"}_Size_Wise_Picklist_${dateText}.xlsx`); setMessage("Excel picklist downloaded.");
  }
  ...
  return <main className="min-h-screen bg-background text-foreground">
    <header className="border-b border-border/80 bg-white/90 backdrop-blur"><div className="mx-auto flex max-w-[1440px] items-center justify-between gap-4 px-5 py-4 lg:px-8">
      <div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl bg-primary text-white"><PackageCheck className="h-5 w-5" /></div><div><p className="font-extrabold tracking-tight">Marketplace Label Sorter</p><p className="text-xs font-medium text-muted-foreground">Meesho + Flipkart · Size → SKU</p></div></div>
    ...
    <section className="mx-auto max-w-[1440px] px-5 py-7 lg:px-8 lg:py-10">
      <div className="mb-7 flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><div className="mb-3 inline-flex items-center gap-2 rounded-full bg-fuchsia-50 px-3 py-1.5 text-xs font-extrabold uppercase tracking-[.13em] text-fuchsia-700"><Sparkles className="h-3.5 w-3.5" />Daily packing workflow</div><h1 className="max-w-3xl text-3xl font-black tracking-[-.04em] sm:text-4xl">Sort shipping labels by Size and SKU.</h1><p className="mt-3 max-w-2xl text-base leading-7 text-muted-foreground">Choose Meesho or Flipkart, upload the original PDF, then download sorted labels and a size-wise Excel picklist.</p></div><div className="flex items-center gap-3 rounded-2xl border border-border bg-card px-4 py-3 shadow-sm"><CalendarDays className="h-5 w-5 text-primary" /><div><p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">{marketplace === "flipkart" ? "Flipkart output" : "Current date"}</p><p className="text-lg font-black tabular-nums">{marketplace === "flipkart" ? "3 × 5 inches" : indiaDate()}</p></div></div></div>
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.5fr)_minmax(340px,.7fr)]">
        <section className="overflow-hidden rounded-[24px] border border-border bg-card shadow-[0_18px_55px_rgba(43,28,48,.08)]">
          <div className="flex items-center justify-between gap-4 bg-[#241826] px-5 py-4 text-white sm:px-6"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-fuchsia-200">Step 1</p><h2 className="mt-1 text-xl font-extrabold">Upload {marketplace === "flipkart" ? "Flipkart" : "Meesho"} label PDF</h2></div>{file && <button onClick={() => inputRef.current?.click()} className="inline-flex items-center gap-2 rounded-xl border border-white/20 px-3 py-2 text-sm font-bold hover:bg-white/10"><RefreshCcw className="h-4 w-4" />Replace</button>}</div>
          <div className="p-5 sm:p-6"><div className="mb-5 grid grid-cols-2 gap-2 rounded-2xl bg-muted p-1.5"><MarketplaceButton active={marketplace === "meesho"} label="Meesho" note="Original labels + date" onClick={() => chooseMarketplace("meesho")} /><MarketplaceButton active={marketplace === "flipkart"} label="Flipkart" note="Crop to 3 × 5 inches" onClick={() => chooseMarketplace("flipkart")} /></div><input ref={inputRef} type="file" accept="application/pdf,.pdf" className="sr-only" onChange={(e) => { const selected = e.target.files?.[0]; if (selected) void analyseFile(selected) }} />
            {!file ? <button onClick={() => inputRef.current?.click()} onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); const selected = e.dataTransfer.files?.[0]; if (selected) void analyseFile(selected) }} className="group grid min-h-[310px] w-full place-items-center rounded-[20px] border-2 border-dashed border-fuchsia-200 bg-[linear-gradient(135deg,#fff8fd_0%,#fff_55%,#fff8ef_100%)] p-8 text-center hover:border-fuchsia-400 focus:outline-none focus:ring-4 focus:ring-fuchsia-100"><span><span className="mx-auto grid h-16 w-16 place-items-center rounded-2xl bg-primary text-white shadow-lg shadow-fuchsia-200 transition group-hover:-translate-y-1"><UploadCloud className="h-7 w-7" /></span><span className="mt-5 block text-xl font-black">Drop PDF here or click to upload</span><span className="mt-2 block text-sm font-medium text-muted-foreground">Original {marketplace === "flipkart" ? "Flipkart A4 invoice + label" : "Meesho shipping-label"} PDF</span></span></button> : <div>
        ...
        <aside className="space-y-5">
          {marketplace === "meesho" ? <section className="rounded-[24px] border border-border bg-card p-5 shadow-[0_18px_55px_rgba(43,28,48,.07)] sm:p-6"><div className="flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-fuchsia-700">Step 2</p><h2 className="mt-1 text-xl font-extrabold">Date position</h2></div><CalendarDays className="h-6 w-6 text-primary" /></div><label className="mt-5 block text-sm font-bold" htmlFor="dateText">Date shown on every label</label><input id="dateText" value={dateText} onChange={(e) => setDateText(e.target.value)} className="mt-2 h-12 w-full rounded-xl border border-border bg-white px-4 text-lg font-black tabular-nums focus:ring-4 focus:ring-fuchsia-100" />
          ...
          </section> : <section className="rounded-[24px] border border-border bg-card p-5 shadow-[0_18px_55px_rgba(43,28,48,.07)] sm:p-6"><div className="flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-fuchsia-700">Step 2</p><h2 className="mt-1 text-xl font-extrabold">3 × 5 label crop</h2></div><FileText className="h-6 w-6 text-primary" /></div><div className="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-4"><p className="flex items-center gap-2 font-extrabold text-emerald-900"><Check className="h-4 w-4" />Flipkart label area detected</p><p className="mt-2 text-sm leading-6 text-emerald-800">The shipping label is cropped from the top of each A4 invoice page and exported at exactly 3 × 5 inches. The invoice section is removed.</p></div><div className="mt-4 grid grid-cols-2 gap-3"><div className="rounded-xl border border-border p-3 text-center"><p className="text-xl font-black">3 in</p><p className="text-xs font-bold uppercase text-muted-foreground">Width</p></div><div className="rounded-xl border border-border p-3 text-center"><p className="text-xl font-black">5 in</p><p className="text-xs font-bold uppercase text-muted-foreground">Height</p></div></div></section>}
          <section className="rounded-[24px] bg-[#241826] p-5 text-white shadow-[0_18px_55px_rgba(43,28,48,.18)] sm:p-6"><p className="text-xs font-bold uppercase tracking-[.14em] text-fuchsia-200">Step 3</p><h2 className="mt-1 text-xl font-extrabold">Download files</h2><p className="mt-2 text-sm leading-6 text-zinc-300">Sort order: XXS → XS → S → M → L → XL → XXL → 3XL → 4XL → 5XL, then SKU.</p><div className="mt-5 grid gap-3"><button disabled={!rows.length || status === "exporting" || !/^\d{2}-\d{2}-\d{4}$/.test(dateText)} onClick={() => void exportPdf()} className="inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-fuchsia-500 px-4 font-extrabold hover:bg-fuchsia-400 disabled:opacity-40">{status === "exporting" ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <ArrowDownToLine className="h-4 w-4" />}Download {marketplace === "flipkart" ? "3×5 " : ""}sorted PDF</button><button disabled={!rows.length} onClick={exportExcel} className="inline-flex h-12 items-center justify-center gap-2 rounded-xl border border-white/20 bg-white/10 px-4 font-extrabold hover:bg-white/15 disabled:opacity-40"><FileSpreadsheet className="h-4 w-4" />Download Excel picklist</button></div><div className="mt-5 grid grid-cols-3 divide-x divide-white/10 rounded-xl border border-white/10 bg-black/10 py-3 text-center text-xs font-bold text-zinc-300"><span>{marketplace === "flipkart" ? "3×5 crop" : "PDF preserved"}</span><span>Size totals</span><span>SKU count</span></div></section>
        </aside>
      </div>
    </section>
  </main>;
...
function MarketplaceButton({ active, label, note, onClick }: { active: boolean; label: string; note: string; onClick: () => void }) {
  return <button type="button" onClick={onClick} className={`rounded-xl px-3 py-3 text-left transition ${active ? "bg-white shadow-sm ring-1 ring-border" : "text-muted-foreground hover:bg-white/60"}`}><span className="block text-sm font-extrabold">{label}</span><span className="text-xs font-medium">{note}</span></button>;
}
